...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, vanilla_anchor): #,xy_loss, wh_loss, no_object_loss, object_loss):
        super(Darknet, self).__init__()
        self.module_defs = parse_model_config(config_path)
        
        self.hyperparams, self.module_list = create_modules(module_defs=self.module_defs,vanilla_anchor=vanilla_anchor) #,xy_loss=xy_loss,wh_loss=wh_loss,no_object_loss=no_object_loss,object_loss=object_loss)
    ...
    def forward(self, x): #, targets=None):
        ...
        return torch.cat(output, 1) #(sum(output), *total_losses) if is_training else torch.cat(output, 1)
